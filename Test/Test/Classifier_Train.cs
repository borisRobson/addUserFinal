...
class Classifier_Train : IDisposable
{
    ...
    {
        _IsTrained = LoadTrainingData(Application.StartupPath + "\\CapturedFaces");
    }
    ...
    public bool Retrain()
    {
        return _IsTrained = LoadTrainingData(Application.StartupPath + "\\CapturedFaces");
    }
    /// <summary>
    /// Retrains the recognizer witout resetting variables like recognizer type.
    /// Takes String input to a different location for training data.
                            ...
                            switch (xmlreader.Name)
                            {
                                ...
                                case "FILE":
                                    if (xmlreader.Read())
                                    {
                                        ...
                                        trainingImages.Add(new Image<Gray, byte>(Application.StartupPath + "\\CapturedFaces\\" + xmlreader.Value.Trim()));
                                    }
                                    break;
                            }
                        }
